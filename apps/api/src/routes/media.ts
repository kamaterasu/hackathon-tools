...
export async function mediaRoutes(app: FastifyInstance) {
  // Proxy media files from MinIO through the API so clients on the local
  // network don't need a direct route to localhost:9000.
  app.get("/api/media/proxy/*", async (req, reply) => {
    const key = (req.params as { "*": string })["*"];
    try {
      const obj = await streamFile(key);
  ...
  app.get("/api/media/:id", async (req, reply) => {
    ...
    const { rows } = await db.query("SELECT * FROM media_items WHERE id = $1", [
      id,
    ]);
    if (!rows.length) return reply.code(404).send({ error: "Not found" });
    const item = rows[0];
    const slides =
      item.type === "pptx" && item.slide_count
        ? Array.from({ length: item.slide_count }, (_, i) =>
            publicUrl(`${item.file_path}-slide-${i + 1}.png`),
          )
        : undefined;
    return reply.send({
      ...item,
      file_url: item.file_path ? publicUrl(item.file_path) : null,
      slides,
    });
  });
  ...
  app.post("/api/media/url", async (req, reply) => {
    const {
      ...
      url,
      duration_seconds = 30,
    } = req.body as { name: string; url: string; duration_seconds?: number };
    const { rows } = await db.query(
  ...
  app.delete("/api/media/:id", async (req, reply) => {
    ...
    if (item.thumbnail_path && item.thumbnail_path !== item.file_path)
      await deleteFile(item.thumbnail_path).catch(() => {});
